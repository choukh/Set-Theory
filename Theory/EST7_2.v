...
Proof with eauto.
  ...
  intros C Hne HsubC.
  pose proof (Hmin C Hne) as [m [Hm Hle]]. eapply sub_trans...
  exists m. split... intros x Hx.
  pose proof (Hle x Hx) as []; [left|right]...
  apply SepI... apply CPrdI; apply HsubC...
Qed.
...
Proof with neauto.
  ...
  - intros Hndc. split... intros B Hne Hsub.
    ...
    split... split... split... eapply sub_trans...
Qed.
...
Proof with auto.
  ...
  apply Hbr in Hp as Hx. apply CPrdE2 in Hx as [Hx _].
  contra as Hx'.
...
Proof with eauto; try congruence.
  ...
  - intros t Ht Hsub. apply SepI...
    ...
    + apply PowerAx. intros p Hp.
      ...
      apply func_ap in Hp... apply CPrdI...
    + exists t. split... split; [|split].
      * apply restr_func...
      * apply restr_dom... eapply sub_trans. apply Hsub.
...
Proof with eauto; try congruence.
  ...
  assert (HL1: ∀ ν₁ ν₂, ∀ t₁ t₂ ∈ A, (t₁ ≤ᵣ t₂) R →
    ...
    assert (Hmt₁: (m ≤ᵣ t₁) R) by (apply (relLe_trans m s t₁ R); auto).
    assert (Hmt₂: (m ≤ᵣ t₂) R) by (apply (relLe_trans m t₁ t₂ R); auto).
    ...
    - apply restrE1 in Hx as [a [b [Ha [Hp Heqx]]]].
      ...
      rewrite Hd₁. apply SepI... apply (relLe_trans a m t₁ R)... left...
      destruct (classic (ν₁[a] = ν₂[a])) as [|Hnq]... exfalso.
      assert (a ∈ B) by (apply SepI; auto).
      apply Hmin in H. eapply lo_not_le_gt...
    - apply restrE1 in Hx as [a [b [Ha [Hp Heqx]]]].
      subst x. apply restrI...
      apply SepE in Ha as [Ha Ham]. eapply dom_binRel in Ha...
      apply func_ap in Hp... apply func_point...
    ...
    assert (Heq2: ν ↾ seg x = F ↾ seg x). {
      ...
      - apply restrE1 in Hp as [a [b [Ha [Hp Heq]]]]. subst p.
        ...
        assert (Hat: (a ≤ᵣ t) R). {
          ...
          apply SepE in Ha as [_ Hax]. left. eapply relLt_le_trans...
        }
        apply func_ap in Hp... apply func_point...
  ...
  assert (HL3: dom F = A). {
    ...
    assert (Hseg: seg t = dom F). {
      ...
      - apply SepE in Hx as [Hx Hxt].
        ...
        apply Hmin in Hxb. eapply lo_not_le_gt...
      - apply Hps in Hx as Hxa. apply segI...
        ...
        apply SepE in Hx as [_ Hxs]. left. eapply relLt_le_trans...
    }
...
Proof with auto.
  ...
  eapply Lt_trans; eauto. apply binRelI...
Qed.
