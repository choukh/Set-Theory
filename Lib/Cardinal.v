...
(* == implicit AC == *)
Lemma ord_le_to_cardLe : ∀ 𝜅 𝜆 ⋵ 𝐂𝐃, 𝜅 ⋸ 𝜆 → 𝜅 ≤ 𝜆.
Proof with auto.
  intros 𝜅 H𝜅 𝜆 H𝜆 Hlt.
...
(* == implicit AC == *)
Lemma cardLe_to_ord_le : ∀ 𝜅 𝜆, 𝜅 ≤ 𝜆 → 𝜅 ⋸ 𝜆.
Proof with auto.
  intros.
  apply cardLe_iff_lt_or_eq in H as [|[_ [_ H]]]...
  left. apply cardLt_to_ord_lt...
Qed.

(* == implicit AC == *)
(* 基数的序与序数的序等价 *)
Fact cardLe_iff_ord_le : ∀ 𝜅 𝜆,
  𝜅 ≤ 𝜆 ↔ 𝜅 ⋵ 𝐂𝐃 ∧ 𝜆 ⋵ 𝐂𝐃 ∧ 𝜅 ⋸ 𝜆.
Proof with auto.
  split.
  - intros Hlt. split. apply Hlt. split. apply Hlt.
    apply cardLe_to_ord_le...
  - intros [H𝜅 [H𝜆 Hlt]]. apply ord_le_to_cardLe...
Qed.
...
Proof with eauto.
  ...
  apply Hleast in H... eapply ord_not_le_gt; revgoals...
Qed.
...
Proof with eauto.
  ...
  intros H𝜅A. apply cardLe_iff in H𝜅A.
  rewrite <- card_of_card in H𝜅A...
  eapply cardLe_to_not_gt...
Qed.
