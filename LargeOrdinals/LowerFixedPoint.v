...
Proof.
  ...
  apply ordMul_eq_0 in H as []; nauto.
Qed.
...
Proof with neauto.
  ...
  - rewrite ordAdd_limit... eapply FUnionI...
    ...
    eapply ordAdd_enlarge_l_strictly...
Qed.
...
(* σ数同时也是满足β < σ₀的所有β-加的不动点 *)
Theorem σ_for_all_smaller_add : ξ ≠ 0 → ∀α ⋵ 𝐎𝐍, ∀β ∈ σ 0, β + σ α = σ α.
Proof with neauto.
  intros Hξ α Hα β Hβ.
  assert (Hoβ: β ⋵ 𝐎𝐍). apply (ord_is_ords (σ 0))...
  rewrite σ_0 in Hβ. apply FUnionE in Hβ as [n [Hn Hβ]].
  rewrite σ_normal_form... ord_ext...
  - eapply ord_trans_le. auto.
    apply ordAdd_preserve_le_l; revgoals. left... 1-3: auto.
    rewrite <- ordAdd_assoc, <- ordMul_distr...
    rewrite (ordAdd_n_absorption ω)...
...
Proof with neauto.
  ...
  - destruct (classic (x = 0)).
    + subst. apply ord_neq_0_gt_0...
      intros H0. apply ordMul_eq_0 in H0 as []...
    + rewrite ordMul_limit... eapply FUnionI...
      apply μ₁E in H as [n [Hn Hx]].
      eapply ordMul_enlarge_l_strictly...
Qed.
...
Proof with neauto.
  ...
  - rewrite ordExp_0_r, <- Heq...
    ...
    + intros H. apply ordMul_eq_0 in H as []...
...
(* μ数同时也是满足0 < β < μ₁的所有β-乘的不动点 *)
Theorem μ_for_all_smaller_mul : ξ ≠ 1 → ∀α ⋵ 𝐎𝐍, ∀β ∈ μ 1, β ≠ 0 → β ⋅ μ α = μ α.
Proof with neauto.
  intros Hξ α Hα β Hβ Hβ0.
  assert (Hoβ: β ⋵ 𝐎𝐍). apply (ord_is_ords (μ 1))...
  rewrite μ_1 in Hβ. apply FUnionE in Hβ as [n [Hn Hβ]].
  destruct (classic (α = 0)). subst. rewrite μ_0, ordMul_0_r...
  rewrite μ_normal_form... ord_ext...
  - eapply ord_trans_le. auto.
    apply ordMul_preserve_le_l; revgoals. left... 1-3: auto.
    rewrite <- ordMul_assoc, ordExp_add...
    rewrite (ordAdd_n_absorption ω)...
  - apply ordMul_enlarge_l... apply ord_suc_correct...
Qed.
