...
Proof with eauto; try congruence.
  ...
  - ω_destruct l... exfalso. eapply Hnq0... subst l.
    ...
    eapply f_injection; eauto; rewrite Hdf; apply Hrh;
      eapply ranI; apply func_correct...
Qed.
...
Proof with eauto; try congruence.
  ...
  - (* C1 ⊆ C2 *) apply InterE in Hc as [_ H]. apply H.
    ...
    + (* A ⊆ C2 *) intros x Hx.
      rewrite <- Hh0 in Hx. eapply FUnionI...
    + (* f⟦C2⟧ ⊆ C2 *) intros y Hy.
...
Proof. exact mult_m_n_0. Qed.

Example ch4_14: ∀n ∈ ω,
  (even n ∨ odd n) ∧ ¬ (even n ∧ odd n).
Proof with eauto; try apply mult_ran; repeat apply ω_inductive; auto.
  intros n Hn. split.
  - set {n ∊ ω | λ n, even n ∨ odd n} as N.
    ω_induction N Hn.
    + left. split... exists 0. split... rewrite mult_m_0...
    + destruct IH.
      * right. split... destruct H as [_ [k [Hk Heq]]].
        exists k. split... rewrite Heq, add_n_1...
      * left. split. apply ω_inductive...
        destruct H as [_ [k [Hk Heq]]].
        exists k⁺. split. apply ω_inductive...
        rewrite Heq, add_m_n, add_m_0...
        rewrite add_n_1, add_n_1, mult_m_n...
        rewrite (add_comm 2)...
        rewrite <- (add_assoc (2⋅k))...
        cut (1 + 1 = 2); try congruence...
        rewrite add_m_n, add_m_0... apply mult_ran...
  - set {n ∊ ω | λ n, ¬ (even n ∧ odd n)} as N.
    ω_induction N Hn; intros [[_ [k [Hk Hkeq]]] [_ [p [Hp Hpeq]]]].
    + rewrite <- add_n_1 in Hpeq...
      exfalso. eapply S_neq_0. rewrite Hpeq...
    + apply IH. split.
      * split... exists p. split...
        rewrite <- add_n_1 in Hpeq... apply S_injection in Hpeq...
      * split... ω_destruct k; subst k.
        rewrite mult_m_0 in Hkeq... exfalso. eapply S_neq_0...
        exists n'. split...
        rewrite mult_m_n, add_comm, add_m_n in Hkeq...
        apply S_injection in Hkeq... apply add_ran...
Qed.
...
Proof with eauto.
  ...
  - exists 0. split... exists 0. split... split.
    rewrite mult_m_0, add_m_0... apply ineq_nq_0_gt_0...
  - apply (IH d Hd) in Hnq0 as [q [Hq [r [Hr [Heq Hrd]]]]]...
    apply ineq_both_side_s in Hrd...
    apply ineq_leq_iff_lt in Hrd as []; try apply ω_inductive...
    + exists q. split... exists r⁺. split.
      apply ω_inductive... split... rewrite add_m_n...
      congruence. apply mult_ran...
    + subst d. exists q⁺. split. apply ω_inductive...
      exists 0. split... split; [|apply empty_in_s; auto].
      rewrite add_m_0, mult_m_n, add_comm, add_m_n; try apply mult_ran...
      congruence. apply ω_inductive...
Qed.

Example ch4_20: ∀ A, A ≠ 0 → A ⊆ ω → ⋃A = A → A = ω.
Proof with eauto.
  intros A Hnq0 HA HU. apply ω_ind... split.
  - apply EmptyNE in Hnq0 as [a Ha].
    destruct (classic (a = 0)). subst a...
    rewrite <- HU. apply UnionAx. exists a. split...
    apply HA in Ha. apply ineq_nq_0_gt_0...
  - intros a Ha. apply HA in Ha as Haw. rewrite <- HU in Ha.
    ...
    + exfalso. apply ineq_leq_iff_lt in H...
...
Proof with eauto.
  ...
  apply H in Hm as Hmm. eapply nat_reg...
Qed.
...
Proof with eauto.
  ...
  - rewrite add_0_n. apply empty_in_s... apply ω_inductive...
  - rewrite add_m_n'... apply (ineq_both_side_s m)...
...
Proof with eauto.
  ...
  - apply ineq_nq_0_gt_0 in H... apply SI in H as [n' [Hn' Heq]]...
    exists n'. split... rewrite add_0_n... congruence.
  - ω_destruct n; subst. exfalso0.
    apply ineq_both_side_s in H...
...
Proof with try apply ω_inductive; auto.
  ...
  rewrite mult_distr... rewrite mult_distr...
  assert (Hmq: m ⋅ q ∈ ω). { apply mult_ran... }
  assert (Hnq: n ⋅ q ∈ ω). { apply mult_ran... }
  assert (Hns: n ⋅ s⁺ ∈ ω). { apply mult_ran... }
  assert (Hms: m ⋅ s⁺ ∈ ω). { apply mult_ran... }
  rewrite add_comm; revgoals... apply add_ran...
  rewrite <- (add_assoc (m⋅q))...
  rewrite (add_comm (m⋅q)); revgoals... apply add_ran...
  apply ineq_both_side_add... apply add_ran... apply add_ran...
  rewrite add_comm... apply ineq_both_side_add...
  apply ineq_both_side_mult... apply S_neq_0.
Qed.
...
Proof with eauto.
  ...
  - ω_destruct a; subst a.
    + exists 0. split. apply S_has_0... rewrite add_m_0...
    + exfalso. apply Hna. apply S_has_0...
  - destruct (classic (m⁺ = a)).
    exists 0. split. apply empty_in_s... rewrite add_m_0...
    apply ω_connected in H as []; try apply ω_inductive...
    exfalso. apply Hna...
    assert (Hma: m ∉ a). {
    ...
    apply ineq_both_side_s in Hnab; try apply add_ran...
    eapply nat_reg. apply Hm. rewrite <- Heq in Hnab...
    apply ω_connected in H0 as []; revgoals... exfalso.
    apply ineq_leq_iff_lt in Hc... eapply not_leq_gt; revgoals...
    exists c⁺. split; revgoals. rewrite add_m_n... congruence.
    apply ineq_both_side_s in H0...
Qed.
...
Proof with eauto; try congruence.
  ...
  assert (Hhf: is_function h). {
    ...
    - rewrite H21 in Hxm1. exfalso.
      ...
      eapply (ineq_leq_iff_neg_lt m Hm (m + b2))...
      apply add_ran... eapply ineq_leq_add_enlarge...
    - rewrite H11 in Hxm2. exfalso.
      assert (Hbw: b1 ∈ ω) by (eapply ω_trans; eauto).
      eapply (ineq_leq_iff_neg_lt m Hm (m + b1))...
      apply add_ran... eapply ineq_leq_add_enlarge...
  ...
  - (* single_rooted h *)
    ...
    + rewrite <- Hfd in Hxm1, Hxm2.
      eapply f_injection; revgoals... split...
  ...
  - apply ExtAx. intros x. split; intros Hx.
    ...
    + assert (Hxd := Hxm). rewrite <- Hfd in Hxd.
      ...
      * intros a. subst a. left.
        split... apply func_ap in Hp...
    + assert (Hxmn := Hx). ω_destruct n.
      rewrite H, add_m_0 in Hx...
      rewrite Hn'eq in Hx.
      ...
      * intros a. subst a. right.
        exists b. split... split... apply func_ap in Hp...
  - apply ExtAx. intros y. split; intros Hy.
    (* ran h ⊆ A ∪ B *)
    ...
    + rewrite <- Hfr in H. apply ranE in H as [x Hp].
      ...
      * apply CProdI... apply domI in Hp.
        rewrite Hfd in Hp. apply ineq_lt_add_enlarge...
    ...
    + rewrite <- Hgr in H. apply ranE in H as [b Hp].
      ...
      * apply CProdI... rewrite add_comm...
        apply ineq_both_side_add; revgoals...
...
Proof with eauto; try congruence.
  ...
  - ω_destruct j2; subst j2... exfalso.
    rewrite mult_m_0, add_0_n in Heq... rewrite Heq in Hlt1.
    apply ineq_lt_add_shrink in Hlt1; try apply mult_ran...
    eapply not_leq_gt; revgoals. apply Hlt1.
    apply ineq_leq_mult_enlarge...
    apply mult_ran... apply Hn.
  - ω_destruct j2; subst j2.
    + exfalso. rewrite mult_m_0, add_0_n in Heq...
      rewrite <- Heq in Hlt2.
      apply ineq_lt_add_shrink in Hlt2;
        try apply mult_ran; try apply ω_inductive...
      eapply not_leq_gt; revgoals. apply Hlt2.
      apply ineq_leq_mult_enlarge...
      apply mult_ran... apply ω_inductive... apply Hn.
    + cut (m = n')... eapply IH...
      do 2 rewrite mult_m_n in Heq...
      do 2 rewrite <- add_assoc in Heq; try apply mult_ran...
      rewrite add_comm, (add_comm n) in Heq;
        try apply add_ran; try apply mult_ran...
      apply add_elim in Heq;
        try apply add_ran; try apply mult_ran...
Qed.
...
Proof with eauto.
  ...
  - assert (Hxw: x ∈ ω). {
    ...
    apply ineq_leq_iff_lt in Hx as []; try apply add_ran...
...
Proof with eauto.
  ...
  - rewrite mult_m_0 in Hx... exfalso0.
  - assert (Hxw: x ∈ ω). {
      eapply ω_trans... apply mult_ran...
      apply ω_inductive...
    }
    rewrite mult_m_n in Hx; try apply mult_ran...
    destruct (classic (x ∈ k)).
    exists x. split... exists 0. split.
    apply S_has_0... rewrite mult_m_0, add_0_n...
    apply ineq_leq_iff_neg_lt in H...
    apply ch4_37_2_0 in Hx as [c [Hc Hx]]; try apply mult_ran...
    apply IH in Hc as [i [Hi [j [Hj Hc]]]]. subst c.
    assert (Hiw: i ∈ ω) by (eapply ω_trans; eauto).
    assert (Hjw: j ∈ ω) by (eapply ω_trans; eauto).
    rewrite add_assoc, <- mult_m_n in Hx; try apply mult_ran...
    exists i. split... exists j⁺. split...
    apply ineq_both_side_s in Hj; try apply ω_inductive...
Qed.
...
Proof with auto.
  ...
  rewrite mult_m_n, (add_comm k); revgoals...
  apply mult_ran...
  apply ineq_leq_iff_lt in Hj as []...
  - apply IH in H. eapply ineq_lt_add_enlarge...
    apply mult_ran...
  - subst. rewrite add_comm, (add_comm (k⋅m))...
    apply ineq_both_side_add... apply mult_ran...
    apply mult_ran... apply mult_ran...
Qed.
...
Proof with eauto; try congruence.
  intros * [Hm [f [[Hff Hfs] [_ [Hfd Hfr]]]]]
           [Hn [g [[Hgf Hgs] [_ [Hgd Hgr]]]]] Hdj.
  split. apply mult_ran...
  set {p ∊ (m ⋅ n) × (A × B) | λ p,
    let x := π1 p in let u := π1 (π2 p) in let v := π2 (π2 p) in
    ∃i ∈ m, ∃j ∈ n, u = f[i] ∧ v = g[j] ∧
    x = m ⋅ j + i
  } as h.
  assert (Hhf: is_function h). {
    ...
    apply CProd_correct in Hy1 as [u1 [Hu1 [v1 [Hv1 Hy1]]]].
    apply CProd_correct in Hy2 as [u2 [Hu2 [v2 [Hv2 Hy2]]]].
    ...
    rewrite H, add_comm, (add_comm (m⋅j2)) in Heq2;
      try apply mult_ran... cut (i1 = i2)...
    eapply add_elim; revgoals... apply mult_ran...
  }
  exists h. split; [split; auto | split; [auto|split]].
  - (* single_rooted h *)
    intros y Hy. split. apply ranE in Hy...
    ...
    apply op_correct in Hy2 as [Hequ Heqv].
    assert (Heqfi: f[i1] = f[i2]) by congruence.
    assert (Heqfj: g[j1] = g[j2]) by congruence.
    apply f_injection in Heqfi; revgoals... split...
    apply f_injection in Heqfj; revgoals... split...
  - apply ExtAx. intros x. split; intros Hx.
    (* dom h ⊆ m ⋅ n *)
    apply domE in Hx as [y Hp]. apply SepE in Hp as [Hp _].
    apply CProdE1 in Hp as []; zfcrewrite.
  ...
  - apply ExtAx. intros y. split; intros Hy.
    ...
    (* ran h ⊇ A ∪ B *)
    assert (Hy' := Hy).
    apply CProd_correct in Hy' as [u [Hu [v [Hv H]]]]. subst y.
