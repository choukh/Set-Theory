...
Proof with eauto.
  ...
  apply ReplE in Hq as [t [Ht Heq]].
  apply CProd_correct in Ht as [r [Hr [s [Hs Ht]]]].
...
Proof with neauto.
  ...
  - pose proof (realE0 _ Hx) as [p [Hp Hpx]]. apply ExtNI.
    ...
    apply ratLt_both_side_add' in Hlt; try assumption.
...
Proof with neauto.
  ...
  - apply SepE in Hp as [Hp Hnp]. apply ratNeg_pos in Hnp.
    ...
    + apply ratLt_both_side_add; try assumption.
      apply ratLt_addInv...
      rewrite <- ratMul_addInv_l...
      rewrite <- (ratMul_ident (-p)) at 2; try assumption.
      apply ratLt_both_side_mul'; try assumption...
      apply ratLt_r2_1.
...
Corollary realLeq_both_side_add_tran : ∀ w x y z ∈ ℝ,
  w ≤ x → y ≤ z → w + y ≤ x + z.
Proof with auto.
  intros w Hw x Hx y Hy z Hz H1 H2.
  apply (realLeq_both_side_add w Hw x Hx y Hy) in H1...
  apply (realLeq_both_side_add y Hy z Hz x Hx) in H2...
  rewrite (realAdd_comm y), (realAdd_comm z) in H2...
  eapply realLeq_tranr; revgoals; eauto; apply realAdd_ran...
Qed.

Corollary realLt_both_side_add_tran : ∀ w x y z ∈ ℝ,
  w <𝐫 x → y <𝐫 z → w + y <𝐫 x + z.
Proof with auto.
  intros w Hw x Hx y Hy z Hz H1 H2.
  apply (realLt_both_side_add w Hw x Hx y Hy) in H1...
  apply (realLt_both_side_add y Hy z Hz x Hx) in H2...
  rewrite (realAdd_comm y), (realAdd_comm z) in H2...
...
Proof with nauto.
  ...
  apply realLeq_both_side_add_tran...
Qed.
...
Proof with nauto.
  ...
  apply realLt_both_side_add_tran...
Qed.
