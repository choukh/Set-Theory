...
Proof with auto; try congruence.
  ...
  - destruct (mul_correct y) as [h [[Hh [Hhd Hhr]] [Hheq _]]]...
    ...
    assert (H1: 1 ∈ ω) by (apply ω_inductive; auto).
    eapply ranI. apply SepI. apply CProdI. apply CProdI.
    apply Hy. apply H1. apply Hy. split; zfcrewrite.
    split... rewrite Pred, mul_n, mul_0, add_m_0...
...
Proof with auto.
  rewrite (Pred 2), add_m_n, add_m_n, add_m_0; try (apply ω_inductive)...
Qed.
...
Proof with auto.
  ...
  rewrite (Pred 2), mul_m_n, mul_m_n, mul_m_0... 
  rewrite add_m_0, add_m_n, add_m_n, add_m_0...
Qed.
...
Proof with auto.
  ...
  set {p ∊ ω | λ p, ∀ m, m ∈ ω → ∀ n, n ∈ ω →
    m ⋅ (n + p) = m ⋅ n + m ⋅ p} as N.
  ω_induction N Hp; intros n Hn k Hk.
  - rewrite add_m_0, mul_m_0, add_m_0... apply mul_ran...
...
Proof with auto.
  ...
  set {p ∊ ω | λ p, ∀ m, m ∈ ω → ∀ n, n ∈ ω →
    m ⋅ (n ⋅ p) = (m ⋅ n) ⋅ p} as N.
  ω_induction N Hp; intros n Hn k Hk.
  - repeat rewrite mul_m_0... apply mul_ran...
  - repeat rewrite mul_m_n... rewrite mul_distr...
    rewrite IH... apply mul_ran... apply mul_ran...
Qed.

Lemma mul_0_n : ∀n ∈ ω, 0 ⋅ n = 0.
Proof with auto.
  intros n Hn.
  set {n ∊ ω | λ n, 0 ⋅ n = 0} as N.
  ω_induction N Hn. rewrite mul_m_0...
  rewrite mul_m_n... rewrite IH... rewrite add_m_0...
Qed.
...
Proof with auto.
  intros n Hn m Hm. ω_destruct m; subst m.
  rewrite mul_0_n, mul_m_0...
  rewrite mul_m_n...
  clear Hm. generalize dependent n'.
  set {n ∊ ω | λ n, ∀ n', n' ∈ ω → n + n ⋅ n' = n'⁺ ⋅ n} as N.
  ω_induction N Hn; intros k Hk.
  - rewrite mul_0_n, mul_m_0, add_m_0... apply ω_inductive...
...
Lemma mul_n_1 : ∀n ∈ ω, n ⋅ 1 = n.
Proof with eauto.
  intros n Hn.
  set {n ∊ ω | λ n, n ⋅ 1 = n} as N.
  ω_induction N Hn.
  - rewrite mul_0_n... apply ω_inductive...
...
Proof with try assumption; try congruence.
  ...
  set {p ∊ ω | λ p, ∀ m, m ∈ ω → ∀ n, n ∈ ω →
    n ^ (m + p) = n ^ m ⋅ n ^ p} as N.
  ω_induction N Hp; intros n Hn k Hk.
  - rewrite add_m_0, exp_m_0, mul_n_1... apply exp_ran...
