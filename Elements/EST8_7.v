...
Proof. intros m Hm n Hn. rewrite fin_ordAdd_eq_add; auto. Qed.
...
Proof. intros m Hm n Hn. rewrite fin_ordMul_eq_mul; auto. Qed.

(* 序数乘方 *)
Definition PreOrdExp := λ α, Operation 1 (λ ξ, ξ ⋅ α).
Notation "α ^ᵖ β" := (PreOrdExp α β) (at level 30) : OrdArith_scope.
...
Lemma preOrdExp_0_r : ∀α ⋵ 𝐎𝐍, α ^ᵖ 0 = 1.
Proof. intros α H. apply operation_0. Qed.
...
Proof with auto.
  ...
  - rewrite preOrdExp_0_r...
Qed.

Lemma preOrdExp_suc : ∀ α β ⋵ 𝐎𝐍, α ^ᵖ β⁺ = (α ^ᵖ β) ⋅ α.
Proof. intros α H. apply operation_suc. Qed.
...
Lemma preOrdExp_limit : ∀α ⋵ 𝐎𝐍, continuous (PreOrdExp α).
Proof. intros α H 𝜆. apply operation_limit. Qed.
...
Proof with nauto.
  ...
  apply preOrdExp_limit...
Qed.

Lemma preOrdExp_ran : ∀ α β ⋵ 𝐎𝐍, α ^ᵖ β ⋵ 𝐎𝐍.
Proof. intros α Hα β Hβ. apply operation_operative; nauto. Qed.
Local Hint Resolve preOrdExp_ran : core.
...
Proof. intros m Hm n Hn. rewrite fin_ordExp_eq_exp; auto. Qed.

(* see also EST8_8.v ordExp_n_ω *)
...
Proof with neauto.
  ...
  - eapply FUnionI... rewrite fin_ordExp_eq_exp...
    apply exp_enlarge_l... apply BUnionI2...
Qed.
...
Fact preOrdExp_0_suc : ∀α ⋵ 𝐎𝐍, 0 ^ᵖ α⁺ = 0.
Proof. intros α Hα. rewrite preOrdExp_suc, ordMul_0_r; auto. Qed.

Fact preOrdExp_0_lim : ∀α ⋵ 𝐎𝐍ˡⁱᵐ, 0 ^ᵖ α = 1.
Proof with neauto.
  intros α Hlim. assert (Hα: α ⋵ 𝐎𝐍)...
  generalize dependent Hlim. generalize dependent α.
  ord_induction. intros α Hα IH Hlim.
  ord_destruct α.
  - subst. rewrite preOrdExp_0_r...
  - exfalso. eapply limord_iff_not_sucord...
  - rewrite preOrdExp_limit... ext.
    + apply FUnionE in H as [β [Hβ H]].
      assert (Hoβ: β ⋵ 𝐎𝐍). eapply ord_is_ords...
      ord_destruct β.
      * subst. rewrite preOrdExp_0_r in H...
      * destruct Hsuc as [γ [Hγ Heq]]. subst.
        rewrite preOrdExp_suc, ordMul_0_r in H... exfalso0.
      * rewrite IH in H...
...
Lemma ordAdd_eq_0 : ∀ α β ⋵ 𝐎𝐍, α + β = 0 → α = 0 ∧ β = 0.
Proof with eauto.
  intros α Hα β Hβ H0.
...
Lemma ordMul_eq_0 : ∀ α β ⋵ 𝐎𝐍, α ⋅ β = 0 → α = 0 ∨ β = 0.
Proof with eauto.
  intros α Hα. ord_induction. intros β Hβ IH H0.
  ord_destruct β. subst...
  - destruct Hsuc as [γ [Hγ Heq]]. subst.
    rewrite ordMul_suc in H0...
    apply ordAdd_eq_0 in H0 as []...
...
Lemma preOrdExp_eq_0 : ∀ α β ⋵ 𝐎𝐍, α ^ᵖ β = 0 → α = 0.
Proof with eauto.
  intros α Hα. ord_induction. intros β Hβ IH H0.
  ord_destruct β.
  - subst. rewrite preOrdExp_0_r in H0... exfalso...
  - destruct Hsuc as [γ [Hγ Heq]]. subst.
    rewrite preOrdExp_suc in H0...
    apply ordMul_eq_0 in H0 as []...
  - rewrite preOrdExp_limit in H0...
    apply union_eq_empty in H0 as [].
...
Lemma ordExp_eq_0 : ∀ α β ⋵ 𝐎𝐍, α ^ β = 0 → α = 0.
Proof with eauto.
  intros α Hα. ord_induction. intros β Hβ IH H0.
  ord_destruct β.
  - subst. rewrite ordExp_0_r in H0... exfalso...
  - destruct Hsuc as [γ [Hγ Heq]]. subst.
    rewrite ordExp_suc in H0...
    apply ordMul_eq_0 in H0 as []...
  - unfold OrdExp in H0. destruct (ixm (α = 0))...
    eapply preOrdExp_eq_0...
Qed.
...
Theorem preOrdExp_normal : ∀α ⋵ 𝐎𝐍, 1 ∈ α → normal (PreOrdExp α).
Proof with neauto.
  intros α Hα Hlt. apply operation_normal...
  intros β Hβ. fold (PreOrdExp α β). fold (PreOrdExp α β⁺).
  rewrite preOrdExp_suc... rewrite <- ordMul_1_r at 1...
  apply ordMul_normal... intros H. apply preOrdExp_eq_0 in H...
  apply ord_gt_1_neq_0_1 in Hlt as []...
Qed.
...
Proof with neauto.
  ...
  apply preOrdExp_normal...
Qed.

Fact ordAdd_is_limord : ∀α ⋵ 𝐎𝐍, ∀𝜆 ⋵ 𝐎𝐍ˡⁱᵐ, 𝜆 ≠ 0 → α + 𝜆 ⋵ 𝐎𝐍ˡⁱᵐ.
Proof with auto.
...
Fact ordMul_is_limord_r : ∀α ⋵ 𝐎𝐍, ∀𝜆 ⋵ 𝐎𝐍ˡⁱᵐ, α ≠ 0 → 𝜆 ≠ 0 → α ⋅ 𝜆 ⋵ 𝐎𝐍ˡⁱᵐ.
Proof with auto.
...
Fact ordMul_is_limord_l : ∀α ⋵ 𝐎𝐍, ∀𝜆 ⋵ 𝐎𝐍ˡⁱᵐ, α ≠ 0 → 𝜆 ≠ 0 → 𝜆 ⋅ α ⋵ 𝐎𝐍ˡⁱᵐ.
Proof with auto.
  intros α Hα 𝜆 H𝜆 H1 H2.
  ...
  - subst. exfalso...
  - destruct Hsuc as [β [Hβ Heq]]. subst.
    rewrite ordMul_suc... apply ordAdd_is_limord...
  - apply ordMul_is_limord_r...
Qed.

Fact ordExp_is_limord_r : ∀α ⋵ 𝐎𝐍, ∀𝜆 ⋵ 𝐎𝐍ˡⁱᵐ, 1 ∈ α → 𝜆 ≠ 0 → α ^ 𝜆 ⋵ 𝐎𝐍ˡⁱᵐ.
Proof with auto.
  intros α Hα 𝜆 H𝜆 H1 H2.
  apply normal_operation_limit_is_limit...
  apply ordExp_normal...
...
Fact ordExp_is_limord_l : ∀α ⋵ 𝐎𝐍, ∀𝜆 ⋵ 𝐎𝐍ˡⁱᵐ, 1 ∈ α → 𝜆 ≠ 0 → 𝜆 ^ α ⋵ 𝐎𝐍ˡⁱᵐ.
Proof with nauto.
  intros α Hα 𝜆 H𝜆 H1 H2.
  assert (Ho𝜆: 𝜆 ⋵ 𝐎𝐍). apply H𝜆.
  ord_destruct α.
  - subst. exfalso0.
  - destruct Hsuc as [β [Hβ Heq]]. subst.
    rewrite ordExp_suc... apply ordMul_is_limord_r...
    intros H. apply ordExp_eq_0 in H...
...
Proof with eauto.
  ...
  - destruct Hsuc as [δ [Hδ Heq]]. subst.
    destruct (classic (α = 0)). subst. rewrite ordExp_0_l...
    ...
    + destruct Hle.
      * left. apply ordMul_normal... intros H1.
        apply ordExp_eq_0 in H1... subst. exfalso0.
      * right. congruence.
  - destruct (classic (α = 0)) as [|Hα0]. subst. rewrite ordExp_0_l...
...
Lemma ordAdd_enlarge_r : ∀ α β ⋵ 𝐎𝐍, β ≠ 0 → α ∈ α + β.
Proof with auto.
  intros α Hα β Hβ H0. rewrite <- ordAdd_0_r at 1...
  apply ordAdd_preserve_lt...
Qed.

Lemma ordAdd_enlarge_l : ∀ α β ⋵ 𝐎𝐍, α ⋸ β + α.
Proof with auto.
  intros α Hα β Hβ. rewrite <- ordAdd_0_l at 1 3...
  apply ordAdd_preserve_le_l...
Qed.
...
Lemma ordMul_enlarge_r : ∀ α β ⋵ 𝐎𝐍, α ≠ 0 → 1 ∈ β → α ∈ α ⋅ β.
Proof with nauto.
  intros α Hα β Hβ Hα0 Hβ1. rewrite <- ordMul_1_r at 1...
  apply ordMul_preserve_lt...
Qed.

Lemma ordMul_enlarge_l : ∀ α β ⋵ 𝐎𝐍, Embed 1 ⋸ β → α ⋸ β ⋅ α.
Proof with nauto.
  intros α Hα β Hβ Hβ1. rewrite <- ordMul_1_l at 1 3...
  apply ordMul_preserve_le_l...
Qed.
...
Lemma ordExp_enlarge_r : ∀ α β ⋵ 𝐎𝐍, 1 ∈ α → 1 ∈ β → α ∈ α ^ β.
Proof with nauto.
  intros α Hα β Hβ Hα1 Hβ1. rewrite <- ordExp_1_r at 1...
  apply ordExp_preserve_lt...
Qed.

Lemma preOrdExp_enlarge_l : ∀ α β ⋵ 𝐎𝐍, 1 ∈ β → α ⋸ β ^ᵖ α.
Proof with auto.
...
Lemma ordExp_enlarge_l : ∀ α β ⋵ 𝐎𝐍, 1 ∈ β → α ⋸ β ^ α.
Proof with auto.
  intros α Hα β Hβ Hβ1.
  apply monotone_operation_weakly_increasing...
  apply ordExp_normal...
...
Proof with neauto.
  ...
  destruct (classic (β = 1)). {
    ...
    intros Hpow. apply ordExp_eq_0 in Hpow...
  }
...
Proof with eauto.
  ...
  destruct (classic (α = 0)) as [|Hα0]. {
    ...
    - intros β [Hβ [γ [[Hγ [Heq _]] _]]]. symmetry in Heq.
      apply ordAdd_eq_0 in Heq as []...
      apply ordMul_eq_0 in H as []... exfalso...
  }
  pose proof (normal_operation_domain_has_maximum (OrdMul δ)) as [β [Hoβ [Hβ Hmax]]].
  auto. apply ordMul_normal... apply Hα. rewrite ordMul_0_r...
...
Proof with neauto.
  ...
  auto. apply ordExp_normal... apply Hα. rewrite ordExp_0_r...
  ...
  - split... pose proof (ord_division α Hα (β ^ γ)) as [δ [[Hδ [ρ [[Hρ [H1 H2]] Hu1]]] Hu2]]...
    intros H0. apply ordExp_eq_0 in H0... subst. exfalso0.
    exists δ. split.
...
Theorem ordExp_add : ∀ α β γ ⋵ 𝐎𝐍, α ^ β ⋅ α ^ γ = α ^ (β + γ).
Proof with eauto.
  intros α Hα β Hβ γ Hγ.
  generalize dependent β.
  generalize dependent α.
...
Theorem ordExp_mul : ∀ α β γ ⋵ 𝐎𝐍, (α ^ β) ^ γ = α ^ (β ⋅ γ).
Proof with neauto.
  intros α Hα β Hβ γ Hγ.
  generalize dependent β.
  generalize dependent α.
