...
(* 单调运算相对自身弱增长（增长或遇到不动点） *)
Lemma monotone_operation_weakly_increasing : 
  ∀ F, F:ᶜ 𝐎𝐍 ⇒ 𝐎𝐍 → monotone F → ∀α ⋵ 𝐎𝐍, α ⋸ F α.
Proof with eauto.
  intros F HF Hmono α Hα. apply ord_le_iff_not_gt...
  intros HFα. apply (woset_no_descending_chain α (MemberRel α)).
  apply ord_woset...
  set (Func α α F) as f.
  assert (Hf: f : α ⇒ α). {
    apply meta_function. intros x Hx.
    apply Hmono in Hx... eapply ord_trans...
  }
  set (ω_Recursion f α (F α)) as g.
  pose proof (ω_recursion_spec_intro f α (F α)) as [Hg [Hg0 Hgn]]... 
  fold g in Hg, Hg0, Hgn.
  exists g. split... intros n Hn.
  assert (Hgnα: g[n] ∈ α). eapply ap_ran...
  apply binRelI... eapply ap_ran... apply ω_inductive...
  rewrite Hgn... unfold f. rewrite meta_func_ap...
  ω_induction n. rewrite Hg0...
  assert (Hgmα: g[m] ∈ α). eapply ap_ran...
  rewrite Hgn... unfold f. rewrite meta_func_ap...
  apply Hmono... apply (ord_is_ords α)...
Qed.
