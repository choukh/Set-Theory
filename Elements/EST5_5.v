...
Require Export ZFC.Elements.EX5.
...
Lemma realLt_trans : tranr RealLt.
...
Lemma realLe : ∀ x y ∈ ℝ, x ≤ y ↔ x ⊆ y.
Proof with auto.
  intros x Hx y Hy. split. apply realLeE. apply realLeI...
Qed.

Lemma realLe_trans : ∀ x y z ∈ ℝ, x ≤ y → y ≤ z → x ≤ z.
Proof with eauto.
  intros x Hx y Hy z Hz H1 H2.
  apply realLe in H1... apply realLe in H2...
  apply realLe... eapply sub_trans...
Qed.

Lemma union_reals_sub_rat : ∀ A, A ⊆ ℝ → ⋃A ∈ 𝒫 ℚ.
Proof with auto.
  intros A H1. pose proof reals_sub_power_rat as H2.
  assert (H3: A ⊆ 𝒫 ℚ) by (eapply sub_trans; eauto).
...
Proof.
  ...
  intros x Hx. apply realLeE. apply Hle. apply Hx.
Qed.
...
Proof with eauto.
  ...
  exists (⋃A). split.
  - repeat split...
    intros x Hxa. apply realLe... apply ex2_3...
  - intros y Hub. apply realLeI... apply Hub.
...
Proof with auto.
  ...
  - apply ReplAx in Hm as [a [Ha Heq]]. apply Hsub in Ha as Haz.
    ...
    rewrite <- Heqa, intAdd_assoc, (intAdd_comm (-b)),
      intAddInv_annih, intAdd_0_r...
  - intros c Hc. apply Hsub in Hc as Hcz.
    apply HA' in Hc as Hcb. apply Hemb in Hc as [n [Hnw Heq]].
    eapply intAdd_preserve_le. apply intAdd_ran... auto. apply Hnb.
    rewrite intAdd_assoc, intAddInv_annih, intAdd_0_r, Heq...
...
Proof with neauto.
  ...
  contra. apply NNPP in H.
  apply Hmin in H. eapply intAdd_preserve_le in H; revgoals.
  apply (int_n 1). apply intAdd_ran... auto.
  rewrite intAdd_assoc, (intAdd_comm (-Int 1)),
    intAddInv_annih, intAdd_0_r in H...
  apply intLt_iff_le_suc in H... eapply intLt_irrefl...
Qed.
...
Proof with neauto.
  ...
  - exists b. split... 
    ...
    assert (p ⋅ IntEmbed [a] <𝐪 r) by (eapply ratLt_trans; eauto).
  ...
  - apply SepE in Hc as [Hcz Hleft].
    ...
    + assert (Hcq: IntEmbed[c] ∈ ℚ) by (apply intEmbed_ran; auto).
      ...
      rewrite <- intEmbed_a, intEmbed, (ratMul_1_r p),
        ratAdd_comm, ratAdd_assoc, (ratAdd_comm (-p) Hnp p Hp),
        (ratAddInv_annih p), ratAdd_0_r; [auto|nauto..].
Qed.
...
Proof with eauto.
  ...
  - assert (Hx' := Hx). assert (Hy' := Hy).
    ...
    eapply ratAdd_preserve_lt_trans...
  ...
  - intros p Hp. apply realAddE in Hp
    ...
    apply ratAdd_preserve_lt_trans; auto;
...
Theorem realAdd_0_r : ∀ x ∈ ℝ, x + Real 0 = x.
Proof with nauto.
  ...
  - apply real_sub_rat in Hp as Hpq... assert (Hp' := Hp).
    ...
    rewrite ratAddInv_annih in Hpr... rewrite (ratAdd_comm p),
      <- ratAdd_assoc, ratAddInv_annih, ratAdd_0_l...
Qed.

Corollary realAdd_0_l : ∀ x ∈ ℝ, Real 0 + x = x.
Proof with nauto.
  intros x Hx. rewrite realAdd_comm, realAdd_0_r...
Qed.
...
Proof with neauto.
  ...
  - apply SepE in Hp as [Hp Hnp]. apply ratNeg_pos in Hnp.
    ...
    assert (Heq: q + (p - q) = p). {
      rewrite (ratAdd_comm p), <- ratAdd_assoc,
        ratAddInv_annih, ratAdd_0_l; auto.
    }
    ...
    + apply ratAdd_preserve_lt; try assumption.
      ...
      rewrite <- (ratMul_1_r (-p)) at 2; try assumption.
...
Proof with auto.
  ...
  rewrite <- (realAdd_0_r (--x)), <- (realAddInv_annih x),
    realAdd_comm, realAdd_assoc, realAddInv_annih, realAdd_0_r...
  apply realAdd_ran...
Qed.
...
Proof with auto.
  ...
  rewrite realAddInv_annih, realAdd_0_r, realAdd_0_r in H...
...
Proof with nauto.
  ...
  rewrite realAdd_comm, realAddInv_annih, realAdd_0_r...
...
Theorem realAdd_preserve_le : ∀ x y z ∈ ℝ,
  x ≤ y → x + z ≤ y + z.
Proof with eauto.
  intros x Hx y Hy z Hz Hle.
...
Proof with eauto.
  ...
  - assert (Hle: x ≤ y) by auto.
    apply (realAdd_preserve_le _ Hx _ Hy _ Hz) in Hle as []...
...
Theorem realSubtr_preserve_le : ∀ x y z ∈ ℝ,
  x + z ≤ y + z → x ≤ y.
Proof with eauto.
  intros x Hx y Hy z Hz [].
...
Corollary realAdd_preserve_le_trans : ∀ w x y z ∈ ℝ,
  w ≤ x → y ≤ z → w + y ≤ x + z.
Proof with auto.
  intros w Hw x Hx y Hy z Hz H1 H2.
  apply (realAdd_preserve_le w Hw x Hx y Hy) in H1...
  apply (realAdd_preserve_le y Hy z Hz x Hx) in H2...
  rewrite (realAdd_comm y), (realAdd_comm z) in H2...
  eapply realLe_trans; revgoals; eauto; apply realAdd_ran...
Qed.

Corollary realAdd_preserve_lt_trans : ∀ w x y z ∈ ℝ,
...
Proof with auto.
  ...
  rewrite (realAdd_comm y), (realAdd_comm z) in H2...
  eapply realLt_trans; revgoals; eauto; apply realAdd_ran...
Qed.
...
Proof with eauto.
  ...
  - apply realLt_connected in H as []...
    ...
    eapply realLt_irrefl.
    eapply realLt_trans; revgoals...
Qed.

Lemma realLe_addInv : ∀ x y ∈ ℝ, x ≤ y → -y ≤ -x.
Proof with auto.
...
Lemma realLe_addInv' : ∀ x y ∈ ℝ, -y ≤ -x → x ≤ y.
Proof with eauto.
  intros x Hx y Hy [Hlt|Heq].
  - left. apply realLt_addInv'...
...
Proof with nauto.
  ...
  - intros p Hp. apply SepE in Hp as [Hpq Hlt].
    apply SepI... eapply ratLt_trans. apply Hlt. apply ratPos_sn.
...
Proof with nauto.
  ...
  unfold realNonNeg. rewrite <- (realAdd_0_r (Real 0))...
  apply realAdd_preserve_le_trans...
Qed.
...
Proof with nauto.
  ...
  rewrite <- (realAdd_0_r (Real 0))...
  apply realAdd_preserve_lt_trans...
Qed.
