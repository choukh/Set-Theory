...
Require Export ZFC.Elements.EST4_2.
...
Proof with nauto.
  ...
  + ω_destruct k.
    * right. apply SepI; zfc_simple. apply CPrdI...
    ...
    * assert (m ≠ k) by congruence.
      ...
      rewrite <- (suc_preserve_lt m Hm k)...
      right. apply binRelE3 in H.
      apply SepI; zfc_simple. apply CPrdI... apply ω_inductive...
      rewrite <- (suc_preserve_lt k Hp m Hm)...
Qed.
...
Proof with eauto.
  ...
  ω_destruct q. exfalso0.
  ω_destruct p. {
    subst. rewrite mul_0_r...
    rewrite mul_suc, add_comm, add_suc...
    apply suc_has_0...
  }
  eapply nat_trans. auto.
  apply mul_preserve_lt; revgoals.
  apply H1. apply suc_neq_0. 1-3: auto.
  apply mul_preserve_lt'...
Qed.
...
Lemma add_enlarge_l : ∀ n k ∈ ω, k ≠ 0 → n ∈ k + n.
Proof with nauto.
  intros n Hn k Hk Hk0.
  rewrite <- add_0_l at 1...
  apply add_preserve_lt... apply neq_0_gt_0...
Qed.

Lemma mul_enlarge_l : ∀ n k ∈ ω, n ≠ 0 → 1 ∈ k → n ∈ k ⋅ n.
Proof with nauto.
  intros n Hn k Hk Hn0 Hk1.
  rewrite <- mul_1_l at 1...
  apply mul_preserve_lt...
Qed.

Lemma exp_enlarge_l : ∀ n k ∈ ω, 1 ∈ k → n ∈ k ^ n.
Proof with nauto.
  intros n Hn k Hk Hlt.
  ω_induction n. rewrite exp_0_r, pred...
  assert (Hkmw: k ^ m ∈ ω)...
  rewrite exp_suc... apply lt_iff_suc_le in IH...
  eapply le_lt_trans. auto. apply IH.
  apply mul_enlarge_l... intros H.
  apply exp_eq_0 in H... subst k. exfalso0.
Qed.

Theorem exp_preserve_exponent_lt : ∀ m n p ∈ ω, 1 ∈ p →
  m ∈ n → p ^ m ∈ p ^ n.
Proof with neauto.
  intros m Hm n Hn p Hp Hp1 H.
  generalize dependent p.
  generalize dependent m.
  ω_induction n; intros k Hk Hle p Hp Hp1. exfalso0.
  rewrite exp_suc...
  apply le_iff_lt_suc in Hle as []...
  - rewrite mul_comm... apply mul_enlarge_lt...
    apply neq_0_gt_0... eapply nat_trans...
  - subst k. apply mul_enlarge_l...
    intros H. apply exp_eq_0 in H... subst p. exfalso0.
Qed.
