...
Require Export ZFC.Elements.EST7_2.
...
Proof with eauto.
  ...
  - apply SepE1 in Hlt...
  - apply SepI... apply CPrdI; apply SepI...
...
Proof with eauto.
  ...
  - apply SepE in Ht₀ as [Ht₀ Hin₀]. assert (H := Hin₀).
    ...
    apply Hmin in Hsb. eapply lo_not_le_gt...
Qed.
  ...
  destruct (classic (e ∈ ran F)) as [He|He]. {
    ...
    assert (HL2: ∀ x y ∈ A S, (x <ᵣ y) (R S) → (y <ᵣ a) (R S) →
      ...
      + exfalso. assert (y ∈ B). apply SepI...
        apply HminS in H. eapply lo_not_le_gt...
