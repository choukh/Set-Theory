...
Require Export ZFC.Elements.EST8_3.
...
Proof with neauto; try congruence.
  ...
  assert (Hbi: F: n × m ⟺ n ⋅ m). {
    ...
    - intros x Hx.
      ...
      set (Embed m) as m'. assert (Hm': m' ∈ ω). apply embed_ran...
      ω_destruct m'... rewrite H in Hb. exfalso0.
      assert (Haw: a ∈ ω). eapply ω_trans...
      assert (Hbw: b ∈ ω). eapply ω_trans...
      rewrite Heq, mul_suc, add_comm; neauto; [|apply mul_ran]...
      assert (Hle: n ⋅ b ⋸ n ⋅ m'). {
        destruct (classic (b = m')) as [|Hnq]. right... left.
        apply mul_preserve_lt'...
        - intros H0. rewrite H0 in Ha. exfalso0.
        - rewrite Heq in Hb. apply BUnionE in Hb as []...
          apply SingE in H...
      }
      destruct Hle.
