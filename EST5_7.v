...
Lemma realLt_supremum : ∀ A, ∀ x z ∈ ℝ,
  x <𝐫 z → supremum z A ℝ RealLt → ∃y ∈ A, x <𝐫 y.
Proof with eauto.
  intros A x Hx z Hz Hxz [Hub Hsup].
  destruct (classic (∃y ∈ A, x <𝐫 y))...
  assert (Hupx: ∀y ∈ A, y ≤ x). {
    intros y Hy. destruct (classic (y = x))...
    apply realLt_connected in H0 as []...
    exfalso. apply H. exists y. split... apply Hub...
  }
  destruct (Hsup x) as []. {
    destruct Hub as [Hpo [Hsub _]]. split...
  }
...
Proof with neauto.
  ...
  pose proof (reals_boundedAbove_has_supremum A) as [s [Hub Hsup]]. {
    ...
    - apply loset_iff_connected_poset. apply realLt_loset.
    - intros w Hw. apply ReplAx in Hw as [a [Ha Heq]].
      subst w. apply Hpdr...
    - intros w Hw. apply ReplAx in Hw as [a [Ha Heq]].
      apply Hleq in Ha. congruence.
  }
  assert (Hub' := Hub). destruct Hub' as [_ [_ [Hs Hle]]].
