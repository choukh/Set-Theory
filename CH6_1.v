...
Proof with eauto; try congruence.
  ...
  assert (Hb: f⁻¹[b] ∈ A ∪ ⎨a⎬). {
    ...
    rewrite <- Hd, <- inv_ran. eapply ranI. apply func_correct.
...
Proof with eauto; try congruence.
  ...
  - intros f Hf. assert (Hh: ℱ f: B ⟺ B). {
    ...
    intros x Hx. rewrite <- Hrh. eapply ranI. apply func_correct...
  ...
  - intros h Hh. apply SepE in Hh as [_ Hh].
    ...
    assert (Hfp: f ∈ Permutation A). {
      ...
      apply Arrow_correct. split; [|split]...
      intros x Hx. rewrite <- Hrf. eapply ranI. apply func_correct...
    }
    exists f. split...
